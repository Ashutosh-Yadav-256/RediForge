...
class AuthService {
    ...
    async verifyGoogleIdToken(tokenString, expectedAudience) {
        ...
        const adminList = (process.env.ADMIN_EMAILS || 'ashutosh4tech@gmail.com')
            .split(',')
            .map(s => s.trim().toLowerCase());
        const userEmail = (payload.email || '').toLowerCase();
        const isAdmin = adminList.includes(userEmail) || userEmail === 'ashutosh4tech@gmail.com';
        ...
        if (!user) {
            ...
            user.role = 'admin';
            if (this.workspaces.has('default-workspace')) {
                this.workspaces.get('default-workspace').members.set(user.id, 'admin');
            }
        }
